...
Description = "ตรวจจับมอนในระยะ 150 studs แล้วโจมตีอัตโนมัติ (ต้องถือขวาน)",
Default = false
})
...
Description = "ตรวจหาต้นไม้ Small Tree รอบตัว 150 studs แล้วตัดอัตโนมัติ (ต้องถือขวาน)",
Default = false
})
...
-- ฟังก์ชั่นเปิด/ปิดบิน
local function SetFly(toggle)
    FlyEnabled = toggle
    hum.PlatformStand = toggle
end
...
RunService.RenderStepped:Connect(function()
    ...
    if FlyEnabled then
        ...
        -- ดึงทิศทางจากระบบเดินของ Roblox (มือถือและ PC ใช้เหมือนกัน)
        local moveInput = UIS:GetGamepadState(Enum.UserInputType.Gamepad1)[1]
        local move = hum.MoveDirection  -- << คีย์สำคัญ ใช้ของ Roblox โดยตรง

        -- ทิศทางตามมุมกล้อง (หน้า / หลัง / ซ้าย / ขวา)
        if move.Magnitude > 0 then
            moveVec += cam.CFrame:VectorToWorldSpace(move)
        end
        ...
        -- ปุ่มนั่งบนมือถือ (DPadDown)
        local gamepad = UIS:GetGamepadState(Enum.UserInputType.Gamepad1)
        if gamepad[1] and gamepad[1].KeyCode == Enum.KeyCode.DPadDown then
            moveVec -= Vector3.new(0,1,0)
        end
...
-----------------------------------------------------------------
-- 🔘 เพิ่มปุ่มเปิด–ปิดบินผ่าน RedzLib
-----------------------------------------------------------------

local ToggleFly = Tab1:AddToggle({
    Name = "✈️ Fly 360° (Mobile/PC)",
    Description = "บินแบบใช้ระบบเดินของ Roblox ควบคุมเต็มรูปแบบ",
    Default = false
})
