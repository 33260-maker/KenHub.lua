...
function redzlib:MakeWindow(Configs)
    ...
    -- ADDED: Track selected values as array for multi-select
    ...
    if MultiSelect then
        for index, Value in pairs(CheckFlag(Flag) and GetFlag(Flag) or Default) do
            if type(index) == "string" and (DOptions[index] or table.find(DOptions, index)) then
                ...
                table.insert(SelectedValues, index) -- ADDED: Track in array
            elseif DOptions[Value] then
                Selected[Value] = true
                table.insert(SelectedValues, Value) -- ADDED: Track in array
            end
        end
    end
    
    -- MODIFIED: Callback now returns array for multi-select
    local function CallbackSelected()
        SetFlag(Flag, MultiSelect and Selected or tostring(Selected))
        Funcs:FireCallback(Callback, MultiSelect and SelectedValues or Selected)
    end
    ...
    -- MODIFIED: Select function to handle both modes
    local function Select(Option)
        if MultiSelect then
            ...
            -- ADDED: Update array of selected values
            if Option.Stats then
                table.insert(SelectedValues, Option.Name)
            else
                table.remove(SelectedValues, table.find(SelectedValues, Option.Name))
            end
        else
            Option.LastCB = tick()
    ...
    AddOption = function(index, Value)
        ...
        if MultiSelect then
            ...
            -- ADDED: Initialize in selected values array
            if Stats then
    ...
    RemoveOption = function(index, Value)
        ...
        if Options[Name] then
            if MultiSelect then 
                Selected[Name] = nil
                -- ADDED: Remove from selected values array
                table.remove(SelectedValues, table.find(SelectedValues, Name))
            else 
                Selected = nil 
            end
...
-- REST OF THE LIBRARY REMAINS EXACTLY THE SAME --
...
-- MODIFIED: Callback returns array for multi-select
...
-- MODIFIED: Select method to handle both modes
